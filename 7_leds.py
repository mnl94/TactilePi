...
def stereo_depth_map(rectified_pair):
    ...
    median_values = process_frame(disparity_fixtype)
    update_leds(median_values)
...
def process_frame(frame):
    ...
    step = width//3
    regions = [frame[:, i*step:(i+1)*step] for i in range(3)]

    median_values = []
    for region in regions:
        intensities = region.flatten()
        intensities.sort()
...
def update_leds(median_values):
    for i, value in enumerate(median_values):
        if value is not None:
            print(value)
            duty_cycle = max(0, min(1, 1-(value/1000)))
            leds[i].value = duty_cycle
            print("led",i,":",' ',duty_cycle,sep='')
        else:
            leds[i].value = 0
            print("led",i,': 0')
